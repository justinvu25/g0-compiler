...
func main() {
	var testresults []time.Duration
	testresults = append(testresults, runtest())
	testresults = append(testresults, runtest())
	testresults = append(testresults, runtest())
	testresults = append(testresults, runtest())
	testresults = append(testresults, runtest())
...
// func NewSymTableEntry(entryType string, name string, tp PrimitiveType, ctp ComplexTypes, lev int, val int, par []SymTableEntry) SymTableEntry {
...
// func NewComplexType(entryType string, fields []SymTableEntry, base PrimitiveType, lower int, length int) ComplexTypes {
// 	return ComplexTypes{
// 		entryType: entryType,
// 		fields:    fields,
// 		base:      base,
...
func statement(inputData *InputData) *SymTableEntry {
	...
	if !exists(inputData.Sym, FIRSTSTATEMENT) {
		...
		//total = append(total, STRONGSYMS...)
		//for !exists(inputData.Sym, total) {
		//	GetSym(inputData)
		//}
	}
	if inputData.Sym == IDENT {
...
func declaration(allocVarLevel string, inputData *InputData) {
	...
	for inputData.Sym == TYPE {
		...
		if inputData.Sym == IDENT {
			...
			//// python simplifies this following block into one line thanks to weak typing.....
			//if x.entryType == "" { //will be blank if it's an array or record
			//	if x.entryType == "record" {
			//		e := Record(x.ctp.fields)
			//		NewDecl(ident, e, inputData)
			//	} else if x.entryType == "array" {
			//		e := Array(x.ctp.base, x.ctp.lower, x.ctp.length)
			//		NewDecl(ident, e, inputData)
			//	}
			//} else {
			//	if x.entryType == "var" {
			...
			//		e := Const(x.tp, x.val)
			//		NewDecl(ident, e, inputData)
			//	} else if x.entryType == "type" {
			//		e := Type(x.tp)
			//		NewDecl(ident, e, inputData)
			//	} else if x.entryType == "proc" {
			//		e := Proc(x.par)
			//		NewDecl(ident, e, inputData)
			//	} else if x.entryType == "stdproc" {
			//		e := StdProc(x.par)
